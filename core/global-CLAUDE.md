...
> 불필요한 확인 요청을 줄이되, 핵심 전제가 불명확한 경우에는 반드시 먼저 질문한다.

### 질문해야 하는 경우 — 구현 전에 반드시 확인

- 요청 대상이 특정되지 않음 — 어떤 API, 어떤 테이블, 어떤 컴포넌트인지 알 수 없을 때
- 요구사항이 서로 모순됨 — 두 조건을 동시에 만족할 수 없을 때
- 되돌리기 어려운 결정이 필요함 — DB 스키마 설계, API 구조처럼 이후 변경 비용이 큰 경우
- 구현 방향에 따라 결과물이 크게 달라짐 — 어떤 방향을 원하는지 컨텍스트에서 파악 불가한 경우

### 질문하지 않는 경우 — 자율 결정 후 완료 보고

- 구현 세부사항 (변수명, 메서드명, 파일 구조 등) — 관례 또는 CLAUDE.md 규칙에 따라 직접 결정
- 여러 구현 방식이 있지만 결과가 동등한 경우 — 직접 선택 후 완료 시 선택 이유 간략히 설명
- CLAUDE.md에 규칙이 이미 명시된 경우 — 다시 묻지 않고 규칙대로 진행
- 컨텍스트에서 이미 파악 가능한 정보 — 재확인 없이 진행

### 질문 방식

- 한 번에 한 가지만 질문한다 — 여러 불명확한 점이 있어도 가장 핵심적인 것 하나만
- 질문과 동시에 내가 가정한 방향을 제시한다 — "A로 이해했는데 맞나요?" 형식
